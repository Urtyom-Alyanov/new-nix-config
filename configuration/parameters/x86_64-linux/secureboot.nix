...
in
  {
    ...
    config = mkIf cfg.enable {
      boot.loader.systemd-boot.extraInstallCommands = ''
        ${mkIf opts.createKeys ''if [ "$(${pkgs.sbctl}/bin/sbctl status --json | ${pkgs.jq}/bin/jq -r '.installed')" = "false" ]; then
          ${pkgs.sbctl}/bin/sbctl create-keys
        fi''}
        ${mkIf opts.enrollKeysOnSetupMode.enable ''if [ "$(${pkgs.sbctl}/bin/sbctl status --json | ${pkgs.jq}/bin/jq -r '.setup_mode')" = "true" ]; then
